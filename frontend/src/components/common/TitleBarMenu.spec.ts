...
describe('桌面顶部段落菜单', () => {
  ...
  it('视图和帮助菜单只连接项目已有页面', async () => {
    ...
    expect(wrapper.text()).toContain('运行日志Benchmark 评测社区目录Function Plot 教程关于 OpenNexus设置与诊断…')
    await wrapper.get('[data-help-function-plot]').trigger('click')
    expect(routerPush).toHaveBeenCalledWith('/help/function-plot')
    await wrapper.get('[data-menu="help"] .menu-trigger').trigger('click')
    await wrapper.get('[data-help-about]').trigger('click')
    expect(routerPush).toHaveBeenCalledWith('/help/about')
    wrapper.unmount()
  })
