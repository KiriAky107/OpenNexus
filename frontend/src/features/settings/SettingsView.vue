...
import StorageLocationSettings from './StorageLocationSettings.vue'
...
<template>
  <section class="feature-page settings-page">
    ...
    <div v-if="activeSection === 'notifications'" class="panel settings-section">
      ...
      <label class="setting-row"><span><strong>{{ t('失败通知', 'Failure notifications') }}</strong><small>{{ t('失败通知会保留，直到手动关闭', 'Failure notifications remain until dismissed') }}</small></span><input v-model="settingsStore.taskFailureNotifications" type="checkbox" :disabled="!settingsStore.taskNotificationsEnabled" /></label>
      <label class="setting-row"><span><strong>{{ t('即将执行提醒', 'Upcoming reminders') }}</strong><small>{{ t('任务将在五分钟内执行时提醒', 'Notify when a task is due within five minutes') }}</small></span><input v-model="settingsStore.taskUpcomingNotifications" type="checkbox" :disabled="!settingsStore.taskNotificationsEnabled" /></label>
    </div>
