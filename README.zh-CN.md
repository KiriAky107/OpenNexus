...
OpenNexus 是一款本地优先的 Windows 桌面应用，用于管理笔记和个人知识。它在同一工作区内提供 Markdown 知识库、混合检索、知识库问答、可审计 Agent、音视频转笔记、扩展和可选的多设备同步。

> 当前版本为 **0.5.2-alpha1**，仍处于早期 Alpha 阶段，存储结构与扩展接口可能继续调整。升级前请备份重要知识库。
...
OpenNexus 将笔记保存为用户拥有的可移植文件，不要求把内容锁在云端服务中。桌面宿主处理本机高权限操作；AI Core 通过受限的本机接口提供检索、生成、Agent、转写和导出。远程服务按需启用，并且独立部署。
...
导入音频或视频后，先查看并校对带时间戳的转录稿，再生成独立的知识点笔记。根据原始内容，笔记可以加入代码、数学公式、Mermaid 图或函数图像。转录稿会保留下来，便于后续复听和核对。
...
为 Agent 设置目标后，可以检查它生成的计划和任务，并只授权必要工具。运行状态会持久化，中断后仍可查看原因并继续处理。
...
- 从经过审查的本地包或社区目录安装 Skill 和 Plugin，不向用户知识库捆绑扩展包。
...
AI Core 由桌面宿主监督运行。独立 MCP 进程不挂在 AI Core 的生命周期下，生产环境也不能依赖仅开发模式可用的 `stdio` 通道。
...
OpenNexus 分开保存用户内容、可重建索引和宿主事务状态。以下存储互有关联，但不共用一个数据库：
...
本仓库只包含桌面主程序和 AI Core。Sync Server 与社区服务的部署代码保留在各自仓库。跨仓库变更需要注明兼容版本，并分别发布。
...
修改依赖清单时，应在同一个 Pull Request 中提交对应的锁定文件。
...
- 不得提交用户 Vault、个人文档、凭据、模型权重或构建产物。
...
如果 Issue 只有“无法使用”等笼统描述、缺少复现信息、与现有问题重复或包含隐私数据，维护者可以暂时关闭，等待补充或修正。
...
9. 不得包含凭据、私人 Vault、个人文档、已下载模型或构建产物。
10. 请求审查前，应通过格式检查、类型检查、测试、生产构建和适用的桌面安装版验收。

早期方案可以使用 Draft Pull Request 讨论。完成检查清单，并确认审查不依赖私人基础设施或个人数据后，再标记为 Ready for review。
